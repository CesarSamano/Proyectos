...
ALTER table IFPE.MTS_REPORTES_REGULATORIOS
    ADD
...
alter table IFPE.MTS_ANA_HRN_CASOS_POLIZAS_AUD add CODOPER varchar(100);

alter table IFPE.MTS_DCONTRATANTE add NUM_CUENTA         varchar(50);
alter table IFPE.MTS_DCONTRATANTE add NUM_CUENTA_CLABE   varchar(50);
alter table IFPE.MTS_DCONTRATANTE add CVE_ORIGEN_CLIENTE varchar(2);

alter table IFPE.MTS_REPORTES_REGULATORIOS ADD   VALOR_51            varchar(500);
alter table IFPE.MTS_REPORTES_REGULATORIOS ADD   VALOR_52            varchar(500);
alter table IFPE.MTS_REPORTES_REGULATORIOS ADD   VALOR_53            varchar(500);
alter table IFPE.MTS_REPORTES_REGULATORIOS ADD   VALOR_54            varchar(500);
alter table IFPE.MTS_REPORTES_REGULATORIOS ADD   VALOR_55            varchar(500);
alter table IFPE.MTS_REPORTES_REGULATORIOS ADD   VALOR_56            varchar(500);
alter table IFPE.MTS_REPORTES_REGULATORIOS ADD   VALOR_57            varchar(500);
alter table IFPE.MTS_REPORTES_REGULATORIOS ADD   VALOR_58            varchar(500);
alter table IFPE.MTS_REPORTES_REGULATORIOS ADD   VALOR_59            varchar(500);
alter table IFPE.MTS_REPORTES_REGULATORIOS ADD   VALOR_60            varchar(500);
alter table IFPE.MTS_REPORTES_REGULATORIOS ADD   VALOR_61            varchar(500);
alter table IFPE.MTS_REPORTES_REGULATORIOS ADD   VALOR_62            varchar(500);
alter table IFPE.MTS_REPORTES_REGULATORIOS ADD   VALOR_63            varchar(500);
alter table IFPE.MTS_REPORTES_REGULATORIOS ADD   VALOR_64            varchar(500);
alter table IFPE.MTS_REPORTES_REGULATORIOS ADD   VALOR_65            varchar(500);
alter table IFPE.MTS_REPORTES_REGULATORIOS ADD   VALOR_66            varchar(500);
alter table IFPE.MTS_REPORTES_REGULATORIOS ADD   VALOR_67            varchar(500);
alter table IFPE.MTS_REPORTES_REGULATORIOS ADD   VALOR_68            varchar(500);
/********************************************************************************************************************************************************************************/
...
CREATE  OR ALTER  VIEW IFPE.MTS_VL_CASOS_INUSUALES_REPORTE_ITF AS
SELECT
    RIGHT('000' + LTRIM(RTRIM((SELECT SECTOR.ORGANISMOSUPID
                                  FROM IFPE.MTS_ADM_SECTOR_CLIENTES SECTOR,
                                    IFPE.MTS_ADM_LICENCIAS LIC
                                  WHERE SECTOR.CVE_SECTOR = LIC.CVE_SECTOR
                                        AND LIC.CVE_ACRONIMO = 'IFPE'))), 3)                       AS  ORGANO_SUPERVISOR,
      '02'                                                                                          AS TIPO_IFPE,
   ( SELECT ENTIDADFINID
                                         FROM IFPE.MTS_ADM_LICENCIAS
                                         WHERE CVE_ACRONIMO = 'IFPE')                              AS  SUJETO_OBLIGADO,
   ...
   -- CTRATE.TIPOPERSONAFISCALID AS CVE_PERSONA_FISCAL,
    (SELECT (case tipopersonafiscalid when 103  then 1 when 104  then 2 when 2  then 2 else 1 end  )
     FROM IFPE.MTS_DTIPOPERSONAFISCAL
    ...
    ISNULL(CTRATE.CONTRATANTEDIRECCION,'')                                                                              AS  CALLE_DOMICILIO,
     (CASE  WHEN DIR.CVE_COLONIA IS NULL
        THEN (SELECT ISNULL(MAX( CVE_ID_ASENTA_CPCONS),'') FROM IFPE.MTS_ADM_SEPOMEX WHERE CVE_D_CODIGO = CTRATE.CODIGO_POSTAL)
    ELSE  RIGHT('0000'+ DIR.CVE_COLONIA,4)END)                                                                          AS  COLONIA,
    ISNULL(ISNULL( (RIGHT('000'+CTRATE.MUNICIPIOID,3))  ,
    (SELECT MAX(CVE_C_MNPIO) FROM IFPE.MTS_ADM_SEPOMEX WHERE CVE_D_CODIGO = CTRATE.CODIGO_POSTAL ))
        ,'')                                                                                                            AS  CIUDAD_POBLACION,
    ...
    FROM IFPE.MTS_KYC_PERSONAS
        WHERE ID_KYC = (SELECT ID_KYC FROM IFPE.MTS_KYC_CONTRATO_PERSONAS
    ...
    (SELECT  isnull(SUM(ISNULL(MONTOCNTR,0)),1.00) FROM IFPE.MTS_HOPERACIONESCNTR
    ...
    AND FECHAOPERACIONCNTR >= DATEADD(MONTH,-1, OPER.FECHAOPERACIONCNTR))                                               AS  MONTO_TOTAL_PERFIL,
     ISNULL((select MONEDAID from (
        ...
        from IFPE.MTS_HOPERACIONESCNTR WHERE CONTRATANTEID = CTRATE.CONTRATANTEID
    ...
    ISNULL((SELECT  COUNT(*) FROM IFPE.MTS_HOPERACIONESCNTR
    WHERE CONTRATANTEID = CTRATE.CONTRATANTEID
   ...
   -- OPER.CONTRATANTECD,
    NULL                                                                                                                AS SW_CASO_MANUAL,
       null as TIPO_SOFOM
FROM IFPE.MTS_HOPERACIONESCNTR OPER,
    IFPE.MTS_DCONTRATANTE CTRATE,
    IFPE.MTS_ANA_HANALISIS_CASOS CASOS,
    IFPE.MTS_HRN_CASOS HCASOS,
    IFPE.MTS_KYC_PERSONAS PER,
    IFPE.MTS_KYC_CONTRATO_PERSONAS CPER,
    IFPE.MTS_KYC_DIRECCIONES DIR
...
CREATE OR ALTER  VIEW [IFPE].[MTS_VL_CASOS_RELEVANTES_REPORTE_ITF] AS
    SELECT
    RIGHT('000' + LTRIM(RTRIM((SELECT SECTOR.ORGANISMOSUPID
                                  FROM IFPE.MTS_ADM_SECTOR_CLIENTES SECTOR,
                                    IFPE.MTS_ADM_LICENCIAS LIC
    ...
    RIGHT('000000' + LTRIM(RTRIM(ISNULL((SELECT ENTIDADFINID
                                         FROM IFPE.MTS_ADM_LICENCIAS
                                         WHERE CVE_ACRONIMO = 'IFPE'),
                                        (SELECT DS_ENTIDADFIN
                                         FROM IFPE.MTS_ADM_LICENCIAS
                                         WHERE CVE_ACRONIMO = 'IFPE')))), 6) 						AS  SUJETO_OBLIGADO,
   -- '65-010'																   AS  SUJETO_OBLIGADO,
    1                                                                          						AS  TIPO_REPORTE,
    CONVERT(CHAR, GETDATE(), 103)                                              						AS  FECHA_REPORTE,
		NULL    														                            AS  FOLIO_CONSECUTIVO,
...
----------------- DATOS DE LA PERSONA
    ...
    /*(SELECT NAC.NACIONALIDADID --Se cambia la clave de la nacionalidad SE DEBE ACTUALIZAR EL CATÁLOGO DE PAÍSES
     FROM IFPE.MTS_DPAIS PA,
       IFPE.MTS_DNACIONALIDAD NAC
...
CREATE or ALTER VIEW [IFPE].[MTS_VL_CASOS_HORAS_REPORTE_ITF] AS
   ...
   -- '65-010'                                                                                                          AS  SUJETO_OBLIGADO,
    ...
    CONVERT(CHAR, GETDATE(), 103)                                                                                       AS  FECHA_REPORTE,
      ...
      '040'+ ISNULL(substring(RIGHT('000000000000000000'+(
          SELECT DS_VALOR FROM IFPE.MTS_KYC_ATRVAL_DATOS_ADICIONALES
    ...
    (SELECT (case tipopersonafiscalid when 103  then 1 when 3  then 1 when 2  then 2 else 1 end  )
     ...
     WHERE TIPOPERSONAFISCALID = CTRATE.TIPOPERSONAFISCALID)   /*OK*/                                                   AS TIPO_PERSONA,
        ''  /*OK PARA IFPE*/                                                                                            AS TIPO_CLIENTE,
    (select  CLAVE_03  from IFPE.MTS_ANA_DCATALOGOS_CLAVES
    ...
    ISNULL(RIGHT('00'+CTRATE.ESTADOID,2),'')                                                                            AS  ENTIDAD_FEDERATIVA_DOMICILIO,
    ISNULL(CTRATE.CONTRATANTEDIRECCION,'')                                                                              AS  CALLE_DOMICILIO,
    (SELECT ISNULL(MAX( CVE_ID_ASENTA_CPCONS),'') FROM IFPE.MTS_ADM_SEPOMEX
        WHERE CVE_D_CODIGO = CTRATE.CODIGO_POSTAL)                                                                      AS  COLONIA,
    ISNULL(ISNULL( (RIGHT('000'+CTRATE.MUNICIPIOID,3))  ,
    (SELECT MAX(CVE_C_MNPIO) FROM IFPE.MTS_ADM_SEPOMEX WHERE CVE_D_CODIGO = CTRATE.CODIGO_POSTAL ))
        ,'')                                                                                                            AS  CIUDAD_POBLACION,
    ISNULL( RIGHT('00000'+CTRATE.CODIGO_POSTAL,5),'')                                                                   AS  CODIGO_POSTAL,
           (select NUM_TEL_CELULAR from IFPE.MTS_KYC_PERSONAS where cve_persona = CTRATE.CONTRATANTECURP   )                                                                                AS  TELEFONO,
   ...
   REPLACE(CONVERT(VARCHAR(40), (CAST(CASOS_MANUALES.MONTOMNCNTR_CASOS AS MONEY)), 1), '.00', '')                       AS MONTO,
    ...
    AND FECHAOPERACIONCNTR >= DATEADD(MONTH,-3, CASOS_MANUALES.FECHADETECION_CASOS))                                    AS  MONTO_TOTAL_PERFIL,
     ISNULL((select MONEDAID from (
        select top(1) MONEDAID,  count (*) as cuenta
        from IFPE.MTS_HOPERACIONESCNTR WHERE CONTRATANTECD = CTRATE.CONTRATANTECD
   ...
   -- OPER.CONTRATANTECD,
    ...
    'REPORTE DE 24 HORAS' AS DS_TIPO_ANALISIS,
          ...
          -- CASOS_MANUALES.NUMPOLIZACNTR_CASOS AS NUMERO_CUENTA,
           ...
           (select DS_TIPO_OPERACION from IFPE.MTS_DTIPO_OPERACIONES  where CASOS_MANUALES.TIPOOPERACIONID = CASOS_MANUALES.TIPO_OPERACION) AS DESCRIPCION_TIPO_OPERACION,
           CASOS_MANUALES.CVE_TIPO_ANALISIS as TIPO_ANALISIS,
           '24 HORAS ' as DESCRIPCION_TIPO_ANALISIS
   ---------------------------------------------------------------------------------------------------------------------------------------------------------------------
    /*FROM IFPE.MTS_ANA_HRN_CASOS_POLIZAS_AUD CASOS_MANUALES, IFPE.MTS_DCONTRATANTE CTRATE
    WHERE CASOS_MANUALES.CVE_TIPO_ANALISIS = 'TA24H'
     AND CASOS_MANUALES.CONTRATANTERFC_CASOS = CTRATE.CONTRATANTERFC*/
FROM IFPE.MTS_ANA_HRN_CASOS_POLIZAS_AUD CASOS_MANUALES
LEFT JOIN IFPE.MTS_DCONTRATANTE CTRATE ON CASOS_MANUALES.CONTRATANTECURP_CASOS = CTRATE.CONTRATANTECURP
WHERE CASOS_MANUALES.CVE_TIPO_ANALISIS = 'TA24H'
go
...
--
CREATE OR ALTER  VIEW [IFPE].[MTS_VL_CASOS_INUSUALES_REPORTE] AS
    ...
    SELECT null as ID_SESION,
           ...
           (SELECT DS_TIPO_OPERACION
            FROM IFPE.MTS_DTIPO_OPERACIONES
           ...
           (SELECT DS_INST_MONETARIO
            FROM IFPE.MTS_DINSTRUMENTO_MONETARIO
           ...
           (SELECT VALOR_ATRIBUTO_01
            FROM IFPE.MTS_ANA_DCATALOGOS_CLAVES
           ...
           'TRANSACCIÓN'                                                                  DS_TIPOINUSUALIDAD,
           HCASOS.OPERCNTRID,
           HCASOS.CODOPER
    FROM IFPE.MTS_HOPERACIONESCNTR OPER,
         IFPE.MTS_DCONTRATANTE CTRATE,
    ...
    WHERE OPER.CONTRATANTEID = CTRATE.CONTRATANTEID
      ...
      AND PER.ID_KYC = CPER.ID_KYC
      AND DIR.ID_KYC = CPER.ID_KYC
      AND DIR.ID_DIRECCION = (select max(D.ID_DIRECCION) from IFPE.MTS_KYC_DIRECCIONES D where d.ID_KYC = Dir.ID_KYC)
      AND CASOS.ESTATUS_OPERACION = 'OP_R'
      AND (CASOS.CVE_TIPO_ANALISIS = 'TAINU' OR CASOS.CVE_TIPO_ANALISIS = 'ALE')
    ...
    SELECT CASOS_MANUALES.ID_SESION,
           ...
           (CASE
                ...
                ELSE
                    (SELECT DISTINCT DS_D_ESTADO
                     FROM IFPE.MTS_ADM_SEPOMEX SEPOMEX
